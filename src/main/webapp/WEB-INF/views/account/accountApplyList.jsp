...
<html>
  <head>
    ...
    <title>계정 신청내역</title>
    <link
      href="${pageContext.request.contextPath}/resources/css/account/accountApplyList.css"
      rel="stylesheet"
    />
    <link
      rel="stylesheet"
          ...
          <div class="content">
            ...
            <div class="d-flex justify-content-end align-items-center">
              <div class="d-flex align-items-center" id="status">
                <label for="statusSelect" class="status-filter-label me-2"
                  >상태</label
                >
                <select
                  id="statusSelect"
              ...
              <div class="d-flex align-items-center">
                <label for="statusSelect" class="status-filter-label" id="opt"
                  >검색조건</label
                >
                <select
                  id="searchSelect"
            ...
            <div id="memberList">
              <div class="d-flex justify-content-start">
                <div>
                  총
                  <span class="form-required" id="memberCount"
                    >${totalRows}</span
                  >건의 신청이 있습니다.
                </div>
              </div>
              <table class="table table-hover table-container">
                ...
                <tbody id="memberTable">
                  <c:forEach items="${list}" var="member" varStatus="status">
                    <tr class="table-row">
                      <td scope="row">
                        ${pager.totalRows - (pager.pageNo-1) * 10 -
                        status.index}
                      </td>
                      <td>
                        <fmt:formatDate
                          value="${member.MApplydate}"
                          pattern="yyyy-MM-dd"
                        />
                      </td>
                      <td>${member.MId}</td>
                        ...
                        <c:choose>
                          <c:when test="${member.MApprovalStatus == 0}">
                            <button
                              ...
                              id="btn-approve"
                              onclick="accountApplyProcess('${member.MId}', 1)"
                            >
                              승인
                            </button>
                            <button
                              class="btn btn-sm me-2"
                              id="btn-reject"
                              onclick="accountApplyProcess('${member.MId}', 2)"
                            >
                              반려
                            </button>
                          </c:when>
              ...
              <div class="page">
                <a
                  href="accountApplyList?pageNo=1"
                  class="btn btn-outline-secondary btn-sm"
                  ><<</a
                >
                <c:if test="${pager.groupNo>1}">
                  <a
                    href="accountApplyList?pageNo=${pager.startPageNo-1}"
                    class="btn btn-outline-dark btn-sm"
                    ><</a
                  >
                </c:if>
                <c:forEach
                  begin="${pager.startPageNo}"
                  end="${pager.endPageNo}"
                  step="1"
                  var="i"
                >
                  <c:if test="${pager.pageNo==i}">
                    <a
                      href="accountApplyList?pageNo=${i}"
                ...
                <c:if test="${pager.groupNo<pager.totalGroupNo}">
                  <a
                    href="accountApplyList?pageNo=${pager.endPageNo+1}"
                    class="btn btn-outline-dark btn-sm"
                    >></a
                  >
                </c:if>
                <a
                  href="accountApplyList?pageNo=${pager.totalPageNo}"
                  class="btn btn-outline-secondary btn-sm"
                  >>></a
                >
              </div>
