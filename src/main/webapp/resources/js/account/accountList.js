...
$(document).ready(function () {
	  $("#memberList").on("click", ".btn-approve", function () {
	    var mId = $(this).data("m-id");
	    var mRole = $(`#mRole-${mId}`).val();
	    var teamId = $(`#teamName-${mId}`).val();
	    var teamName = $(`#teamName-${mId} option:selected`).text();
	    ...
	    $.ajax({
	      ...
	      success: function (data) {
	        Swal.fire({
	          ...
	          location.href = `/Metamong/account/accountList?option=${option}&keyword=${keyword}&pageNo=${pageNo}`;
	        });
	      },
	    });
	  });
...
// 회원 정보 수정
	...
	$(".teamId").each(function () {
	    const $select = $(this);
	    const memberTeamName = $select.data("team-name"); 
	    ...
	    $.ajax({
	        ...
	        data.forEach((team) => {
	        	$select.append(
	        			  `<option value="${team.teamId}" ${
	        			    team.teamName == memberTeamName ? "selected" : ""
	      ...
	      error: function (jqXHR, textStatus, errorThrown) {
	        console.error("Error fetching team list:", textStatus, errorThrown);
	      },
	    });
	  });
	}
